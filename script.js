...
const input = document.getElementById('city_input');
    ...
    input.addEventListener('input', () => {
      const show = input.value.trim().length > 2;
...
const getWeatherByCityName = async (city = "ahmedabad") => {
    try {
        ...
        data.forecast.forecastday.map((item) => {
            return (
                cardContainer.innerHTML += `<div class="w-full flex flex-col items-center justify-center gap-1 bg-slate-200 text-slate-900 rounded-3xl py-2 px-4">
...
const getWeatherByCurrentLocation = async (latitude, longitude) => {
    try {
        ...
        data.forecast.forecastday.map((item) => {
            return (
                cardContainer.innerHTML += `<div class="w-full flex flex-col items-center justify-center gap-1 bg-slate-200 text-slate-900 rounded-lg p-4 shadow-2xl shadow-black/50
">
                        <h4 class="font-bold">(${item.date})</h4>
                        <img src="https:${item.day.condition.icon}" alt="${item.day.condition.text}" width="70" height="70" />
                        <p class="self-start font-semibold">Temp:<span class="text-sm ml-1 font-normal">${item.day.avgtemp_c}<sup>o</sup> C</span></p>
